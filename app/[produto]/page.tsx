...
type ProdutoConfig = {
  slug: string;
  titulo: string;
  /** H1 da página, orientado ao benefício do cliente (Sprint 3 — auditoria de design). */
  headline: string;
  subtitulo: string;
  descricao: string;
  icone: string;
...
const PRODUTOS: Record<string, ProdutoConfig> = {
  "seguro-auto": {
    slug: "seguro-auto",
    titulo: "Seguro Auto",
    headline: "Seu carro protegido, sem pagar a mais por isso",
    subtitulo: "Compare mais de 40 seguradoras em minutos",
    descricao:
  ...
  "seguro-residencial": {
    ...
    headline: "Sua casa protegida, do seu jeito",
    subtitulo: "Monte a cobertura certa para o seu imóvel",
    descricao:
  ...
  "seguro-viagem": {
    ...
    headline: "Viaje tranquilo, do check-in à volta pra casa",
    subtitulo: "Cobertura completa para qualquer destino",
    descricao:
    ...
    <section className="bg-[#535391] text-white">
      <div className={`max-w-5xl mx-auto px-6 ${heroPadding} grid md:grid-cols-2 gap-8 md:gap-12 items-center`}>
        ...
        <div>
          ...
          <p className="text-[#5CBECB] font-bold text-sm uppercase tracking-widest mb-2">
            {config.titulo}
          </p>
...
export default async function ProdutoPage({
  ...
  return (
    <main className="min-h-screen scroll-smooth font-[family-name:var(--font-nunito)]">
      ...
      <Footer
        ctaWhatsapp="Ainda com dúvidas? Fale direto com um corretor."
        origemWhatsapp={`rodape_${produto}`}
      />
    </main>
  );
}
